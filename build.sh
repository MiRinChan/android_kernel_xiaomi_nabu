...
while [ $# != 0 ]; do
    case $1 in
        ...
        "--nolto") no_thinlto=true;;  # 通过此参数禁用 ThinLTO
        "--ksu") with_ksu=true;;
        "--") {
            shift
            while [ $# != 0 ]; do
...
operate:
    ...
    --nolto    : disable ThinLTO (now enabled by default)
...
if [ -f $ZIMG ]; then
    ...
    # 合并 DTB 文件
    DTB_FILES=(
        "$DTB_SOURCE_DIR/sm8150-v2-xiaomi.dtb"
        "$DTB_SOURCE_DIR/sm8150-xiaomi.dtb"
        "$DTB_SOURCE_DIR/sm8150p-v2-xiaomi.dtb"
        "$DTB_SOURCE_DIR/sm8150p-xiaomi.dtb"
    )
